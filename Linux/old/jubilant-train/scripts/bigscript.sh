...
# NOTES
# Changes all passwords to the $password variable below
# Disables the root account
# 
# Files needed:
# - ./users.txt: List of allowed users.  If a user is not in this list and has a UID >= 1000, they will be removed.
# - ./admins.txt: List of users who should have sudo access.  Users not in this list are removed from the sudo group if their uid is >= 1000.
# - ./10periodic: The APT periodic configuration to overwrite the existing config with.
# - ./before.rules: The UFW rules to use.
# - ./sshd_config: The sshd config to use.
# - ./sshd: The sshd PAM configuration to use.
# - ./login.defs: The login.defs file to use.
# - ./common-auth: The pam common-auth file to use.
# - ./common-password: The pam common-password file to use.
# - ./software.txt: Packages to purge from the system
# - ./services.txt: Systemd services to disable.
# - ./sysctl.conf: Sysctl config to use
# - ./host.conf: Hosts file to use
# - ./fstab: fstab file to use
# - ./audit.rules: Default auditd settings
# - ./limits.conf: Security limits to use
...
while read user; do
  ...
  # Setting the expiry date of the password
  chage -m 7 -M 90 -W 14 $user &> /dev/null
done < users.txt
...
while read -r user pass uid gid desc home shell; do
  # Skipping system users (uid < 1000).  Review manually later
  if (($uid >= 1000)); then
...
echo -e "[ i ] Cleaning unused dependencies..."
apt-get autoremove -y > /dev/null
overwrite "${YES} Cleaned unused dependencies"

echo -e "[ i ] Updating APT periodic configuration..."
cp /etc/apt/apt.conf.d/10periodic 10periodic.bak > /dev/null
...
echo -e "[ i ] Ensuring iptables-persistent is not also installed"
apt purge -y iptables-persistent > /dev/null
overwrite "${YES} Ensured iptables-persistent is not installed"

echo -e "[ i ] Enabling UFW service..."
systemctl unmask ufw.service > /dev/null
systemctl --now enable ufw.service > /dev/null
ufw enable > /dev/null
ufw logging full > /dev/null
echo -e "${YES} UFW enabled"
...
cp /etc/audit/audit.rules audit.rules.bak > /dev/null
...
# Using debsums
sudo debsums -c 2>&1 | sudo tee ~/modified_binaries.txt >/dev/null
sudo debsums -ce 2>&1 | sudo tee ~/modified_configs.txt >/dev/null
...
cp /etc/pam.d/common-password common-password.bak
...
echo -e "[ i ] Setting passwd file permissions..."
chown root:root /etc/passwd
chmod 644 /etc/passwd
overwrite "${YES} Set passwd file permissions"
